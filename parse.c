...
static node_t *build_leaf(void) {
    node_t *myleaf = calloc(1, sizeof(node_t));
        if (! myleaf) {
        // calloc returns NULL if memory allocation fails
        logging(LOG_FATAL, "failed to allocate node");
        return NULL;
        }
    myleaf->tok=this_token->ttype;
    myleaf->node_type = NT_LEAF;
    //do I define the data value here too?
   if (this_token->ttype == TOK_NUM){
       myleaf->type = INT_TYPE;
   }
   if(this_token->ttype == TOK_STR){
       myleaf->type = STRING_TYPE;
   }
   //if(this_token->ttype == TOK_TRUE || this_token->ttype == TOK_FALSE){
     //  myleaf->type = TOK;
  // }

   if (this_token->ttype == TOK_NUM){
      myleaf->val.ival = atoi(this_token-> repr);
   }
    //printf("%s", this_token->repr);
    //advance_lexer();
...
static node_t *build_exp(void) {
    ...
    new_node->node_type = NT_INTERNAL;
    new_node->type = INT_TYPE;
    if (this_token->ttype == TOK_LPAREN) {
        advance_lexer(); 
        new_node->children[0] = build_exp();
        advance_lexer();
        new_node->tok = this_token->ttype;
        advance_lexer();
        new_node->children[1] = build_exp();
        advance_lexer(); 
...
static node_t *build_root(void) {
    logging(LOG_INFO, "testing build_root");
    // check running status
    if (terminate || ignore_input) return NULL;
...
node_t *read_and_parse(void) {
    init_lexer();
    // printf("%d", this_token->startpos);
    // printf("%d", next_token->startpos);
    return build_root();
}
